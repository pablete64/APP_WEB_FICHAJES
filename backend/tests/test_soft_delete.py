...
def test_user_soft_delete(db_session):
    # 1. Create a user
    user_in = UserCreate(employee_code="soft_user_test", name="Soft Test", is_admin=False, password="password")
...
def test_time_entry_soft_delete(db_session, normal_user, seed_projects, seed_tasks, assign_project_role):
